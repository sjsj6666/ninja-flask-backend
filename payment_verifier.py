...
def strip_html_tags(text):
    """Removes HTML tags and cleans up whitespace to make regex easier."""
    # Remove HTML tags
    clean = re.sub('<[^<]+?>', ' ', text)
...
def parse_payment_email(email_body):
    try:
        # 1. Clean the body (Convert HTML to flat text)
        clean_body = strip_html_tags(email_body)
        
        # Log cleaned body for debugging if needed (truncated)
        logging.info(f"DEBUG CLEAN BODY: {clean_body[:300]}...")
        ...
        # 4. Extract Name (From clean text)
        # Looks for "From: NAME" until it hits a keyword like "If this", "Transaction", "Amount"
        from_name = "N/A"
...
def names_are_equivalent(bank_name, user_name):
    ...
    # 1. Sequence Insensitive (e.g. Tan David == David Tan)
    if sorted(bank_words) == sorted(user_words):
        return True
        
    # 2. Space Insensitive (e.g. DavidTan == David Tan)
        ...
        for msg_id in message_ids:
            ...
            for response_part in msg_data:
                if isinstance(response_part, tuple):
                    ...
                    body = ""
                    if msg.is_multipart():
                        # Prioritize HTML if available to ensure we capture structure, but plain text is fine too
                        # We will strip tags anyway.
                        for part in msg.walk():
                            if part.get_content_type() == "text/html":
                                body = part.get_payload(decode=True).decode(errors='ignore')
                                break
                        if not body: # Fallback to text/plain
                            for part in msg.walk():
...
def update_order_status(details):
    ...
    try:
        ...
        if from_name and from_name != "N/A":
            ...
            # Fallback only if regex failed entirely
...
if __name__ == "__main__":
    ...
    while True:
        process_emails()
        time.sleep(CHECK_INTERVAL_SECONDS)# payment_verifier.py
