...
BACKEND_URL = "https://api.gameuniverse.co"
...
{"server_id": 1, "server_name": "Prontera-(13)/Izlude-9(-10)/Morroc-(110)"},
{"server_id": 4, "server_name": "Prontera-(46)/Prontera-10/Izlude-(18)"},
{"server_id": 7, "server_name": "Prontera-(79)/Geffen-(110)/Payon-(110)"},
{"server_id": 51, "server_name": "Poring Island-(110)/Orc Village-(110)/Shipwreck-(19)/Memoria/Awakening/Ant Hell-(1~10)/Goblin Forest-1(-2-4)/Valentine"},
...
try:
detail_resp = gp.request("product/detail", {"token": token, "productid": product['id']})
if detail_resp.get('code') == 200:
...
yield ",".join(row) + "\n"
return Response(stream_with_context(generate_csv()), mimetype="text/csv", headers={"Content-Disposition": f"attachment; filename=gamepoint_catalog{gp.config['mode']}.csv", "Cache-Control": "no-cache"})
except Exception as e:
...
@app.route('/api/admin/order/inquiry/<order_id>', methods=['POST'])
@admin_required
def manual_order_inquiry(order_id):
try:
res = supabase.table('orders').select('supplier_ref').eq('id', order_id).single().execute()
if not res.data or not res.data.get('supplier_ref'):
return jsonify({"status": "error", "message": "No supplier reference"}), 404
supplier_ref = res.data['supplier_ref']
gp = GamePointService(supabase_client=supabase)
ref_to_query = supplier_ref.split(',')[0].strip()
inquiry_resp = gp.check_order_status(ref_to_query)
if inquiry_resp.get('code') == 100:
voucher_data = {"pin1": inquiry_resp.get('pin1'), "pin2": inquiry_resp.get('pin2'), "message": inquiry_resp.get('message')}
supabase.table('orders').update({'status': 'completed', 'voucher_codes': voucher_data}).eq('id', order_id).execute()
return jsonify({"status": "success", "data": voucher_data})
return jsonify({"status": "error", "message": inquiry_resp.get('message', 'Still processing')})
except Exception as e:
return jsonify({"status": "error", "message": str(e)}), 500
...
payload = {
'amount': "{:.2f}".format(real_amount), 'currency': 'SGD', 'reference_number': order_id,
...
customer_name = order.get('remitter_name')
...
all_success, _ = False, failed_items.append(f"{item.get('name')} (Error)")
if all_success:
supabase.table('orders').update({'status': 'completed', 'supplier_ref': ', '.join(supplier_refs)}).eq('id', order_id).execute()
send_order_update({**order, 'status': 'completed'}, product.get('name'), game.get('name'), customer_email, customer_name)
else:
status_to_set = 'manual_review' if failed_items else 'processing'
supabase.table('orders').update({'status': status_to_set, 'notes': f"Log: {'; '.join(failed_items)}", 'supplier_ref': ', '.join(supplier_refs)}).eq('id', order_id).execute()
else:
...
supabase.table('orders').update({'status': 'manual_review', 'notes': f"Failed: {create_resp.get('message')}"}).eq('id', order_id).execute()
else:
...
order_id_prefix = merchant_code.split('-')[0]
order_res = supabase.table('orders').select('*').ilike('id', f"{order_id_prefix}%").execute()
