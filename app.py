...
# Suppress the InsecureRequestWarning that will be generated by verify=False
import urllib3
...
def create_paynow_qr():
    ...
    try:
        ...
        response = requests.post(third_party_url, data=form_data, headers=headers, timeout=20, verify=False)
        response.raise_for_status()

        api_response_data = response.json()

        if api_response_data.get("success") and api_response_data.get("qr_image"):
            logging.info(f"Successfully received branded QR image from third-party for order: {order_id}")
            return jsonify({
