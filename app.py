...
SMILE_ONE_HEADERS = {
    ...
    "Referer": "https://www.smile.one/ru/merchant/genshinimpact",  # Updated for Genshin
...
def check_smile_one_api(game, uid, server_id):
    ...
    params = {
        "uid": uid,
        "sid": server_id,  # Using 'sid' as per your request data
        "pid": "19731" if game == "genshin-impact" else "25",  # Genshin PID from your data
        "checkrole": "1",
        "pay_method": "",  # Kept empty as in MLBB
        "channel_method": ""  # Kept empty as in MLBB
    }
    ...
    try:
        ...
        print(f"Smile One Raw Response for {game}: {raw_text}")
        data = response.json()
...
def check_smile_one(game, uid, server_id):
    ...
    # Return 'username' for MLBB compatibility, full result for Genshin
