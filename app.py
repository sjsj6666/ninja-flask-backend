...
def create_hitpay_payment():
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            return jsonify({'status': 'error', 'message': response_data.get('message', 'Failed to create payment request')}), 400
...
def hitpay_webhook_handler():
    try:
        ...
        logging.info(f"Received HitPay webhook for reference: {form_data.get('reference_number')}")
        ...
        if HITPAY_SALT:
            ...
            if hitpay_signature and generated_signature != hitpay_signature:
                logging.warning(f"Signature Mismatch! HitPay: {hitpay_signature} | Ours: {generated_signature}")
