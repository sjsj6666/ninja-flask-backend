...
def check_game_id(game_slug_from_frontend, uid, server_id):
    ...
    smileone_games_map = { "honkai-star-rail": "honkaistarrail", "bloodstrike": "bloodstrike", "ragnarok-m-classic": "ragnarokmclassic", "love-and-deepspace": "loveanddeepspace", "bigo-live": "bigolive" }
    razer_games_map = { "genshin-impact": "genshin-impact", "zenless-zone-zero": "zenless-zone-zero", "ragnarok-origin": "ragnarok-origin", "snowbreak-containment-zone": "snowbreak" }
    game_handlers = {
    ...
    handler = None
    if game_lower in razer_games_map: handler = lambda: check_razer_api(razer_games_map[game_lower], uid, server_id)
    elif game_lower in smileone_games_map: handler = lambda: check_smile_one_api(smileone_games_map[game_lower], uid, server_id)
    else: handler = game_handlers.get(game_lower)

    if handler: result = handler()
    else: result = {"status": "error", "message": f"Validation not configured for game: {game_slug_from_frontend}"}
